...
export class BillingService {
  ...
  async getStatus(protocolId: string) {
    ...
    const subscription =
      await this.subscriptionRepo.findByProtocolId(protocolId);

    const sendsLimit = TIER_SEND_LIMITS[protocol.tier] || 1000;
    const sendsThisPeriod = Number(
      subscription?.sendsThisPeriod ?? protocol.sendsThisPeriod,
    );
  ...
  async getPaymentHistory(
    ...
    const [payments, total] = await this.paymentRepo.findByProtocolId(
      protocolId,
      skip,
      limit,
    );
