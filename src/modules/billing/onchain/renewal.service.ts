...
export class OnChainRenewalService {
    ...
    async renewSubscription(protocolOwner: PublicKey): Promise<string> {
        ...
        const authorityClient = new AuthorityClient({
            ...
            cluster: this.config.get<string>('NODE_ENV') === 'production' ? 'mainnet-beta' : 'devnet',
        });
    ...
    private async getAuthorityKeypair(): Promise<Keypair> {
        ...
        if (isDev && devPath) {
            try {
                ...
                this.logger.warn(`Failed reading dev keypair at ${devPath}. Generating random keypair for auth stub.`);
        ...
        if (!kmsKeyId) {
            this.logger.warn('No HERALD_AUTHORITY_KMS_KEY_ID in PROD. generating random keypair.');
            return Keypair.generate();
        }
