...
import { HelioClient, HelioCheckoutResult, HelioWebhookPayload, HelioSubscriptionTemplate } from '@herald-protocol/sdk/billing';
...
export class HelioService {
    ...
    async createCheckoutUrl(
        ...
        if (tier === 0) {
            throw new BadRequestException('Developer tier is free — no checkout needed');
        }
        ...
        const templateId = this.config.getOrThrow<string>(
            tier === 1 ? 'HELIO_TEMPLATE_GROWTH' :
                tier === 2 ? 'HELIO_TEMPLATE_SCALE' :
    ...
    parseAndVerifyWebhook(rawBody: string, signature: string): HelioWebhookPayload | null {
        try {
