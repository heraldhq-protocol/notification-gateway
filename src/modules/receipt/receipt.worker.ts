...
export class ReceiptWorker extends WorkerHost {
    ...
    private initClient() {
        try {
            ...
            const rpcUrl = this.config.get<string>('SOLANA_RPC_URL') || 'http://localhost:8899';
            this.authorityClient = new AuthorityClient({
                rpcUrl,
                programId: this.config.get<string>('HERALD_PROGRAM_ID') || '2pxjAf8tLCakKVDuN4vY51B5TeaEQk4koPuk9NZvWqdf',
    ...
    async process(job: Job<any, any, string>): Promise<void> {
        ...
        if (job.name === 'flush-receipts') {
            ...
            if (!this.authorityClient) {
                this.logger.error('AuthorityClient not initialized. Cannot flush receipts.');
            ...
            const outputTreeStr = this.config.get<string>('LIGHT_OUTPUT_TREE') || PublicKey.default.toBase58();

            for (const notification of notifications) {
                try {
                    const protocolOwner = (await this.prisma.protocol.findUnique({ where: { id: notification.protocolId } }))?.protocolPubkey;
                    if (!protocolOwner) continue;

                    // 1. Fetch ValidityProof from Light Protocol
                    const validityProof = await this.lightClient.getValidityProof(outputTreeStr);

                    // 2. Parse hash to bytes
                    const recipientHashBytes = Buffer.from(notification.walletHash, 'hex'); // Assuming hex hash

                    // Format notification ID as 16 bytes (UUID parsing omitted for brevity, assuming simple bytes here)
                    const notificationIdBytes = Buffer.from(notification.id.replace(/-/g, '').padEnd(32, '0'), 'hex');
                    ...
                    const ix = await this.authorityClient.writeReceipt({
                        ...
                        lightRemainingAccounts: validityProof.remainingAccounts.map((acc: any) => ({
                    ...
                    this.logger.error(`Failed to write receipt for ${notification.id}: ${(error as Error).message}`);
